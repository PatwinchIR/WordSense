...
def queryset_foreach(queryset, f, batch_size=1000,
    ...
    * stable even if items are added or removed during processing
    ...
    if settings.DEBUG:
        #print >> sys.stdout, 'Warning: DEBUG is on. django.db.connection.queries may use up a lot of memory.'
        print('Warning: DEBUG is on. django.db.connection.queries may use up a lot of memory.')
    ...
    with open("failed_ids.txt", 'w') as fileObj:
        fileObj.write('\n'.join(map(lambda x: str(x), status.failed_ids)))
